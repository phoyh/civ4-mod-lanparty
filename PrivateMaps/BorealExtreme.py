...
class BorealTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	...
	def initFractals(self):
		...
		self.iIce = self.ice.getHeightFromPercent(58) # was 86 -> tripled ice occurrence (goal: empty landscapes between settlements)
		self.iPeaks = self.plains.getHeightFromPercent(82) # was ice edge - no added value to iIce -> use for peaks instead and triple occurrence (goal: manoeuvring and choke points, especially important in the absence of much water)
		self.iPlains = self.plains.getHeightFromPercent(10) # was 21 -> halved plains occurrence
		...
		self.terrainPeak = self.gc.getInfoTypeForString("TERRAIN_PEAK")
	...
	def generateTerrainAtPlot(self,iX,iY):
		if (self.map.plot(iX, iY).isWater()):
			...
			elif plainsVal >= self.iPeaks:
				terrainVal = self.terrainPeak
