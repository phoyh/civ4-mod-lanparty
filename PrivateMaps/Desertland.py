...
def getWrapX():
	#return False
	#OYH: horizontal wrap appropriate - can find Niles in any direction - barbarians can also come from W or E
	return True
def getWrapY():
	return False
...
class OasisTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	...
	def generateTerrainAtPlot(self,iX,iY):
		...
		if lat > self.iOasisTopLatitude:
			# OYH: No plains/grass on the top
			pass
...
class OasisFeatureGenerator(CvMapGeneratorUtil.FeatureGenerator):
	def __init__(self, iJunglePercent=2, iForestPercent=5,
...
def addBonusType(argsList):
	...
	if (type_string in resourcesToEliminate):
		...
		# while count > 0:
		while count > 0 and len(eligible) > 0:
			# OYH: robust while loop if nobody eligible
