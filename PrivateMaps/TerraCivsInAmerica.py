...
def findStartingPlot(argsList):
	map = CyMap()
	eurasiaID = map.findBiggestArea(False).getID()
	...
	for area in areas:
		...
		if (area.getID() == eurasiaID): continue
		numTiles = area.getNumTiles()
		if (numTiles < maxTiles): continue
		maxTiles = numTiles
		secondAreaID = area.getID()
